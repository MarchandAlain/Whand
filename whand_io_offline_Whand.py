...
File_extension=".e01"                                                           # result file to analyze
Exit_delay=100                                                                    # 1/10e s to conclude experiment 
Svlist=[None]
...
def readpins(sv):                                                                   # required by Whand
    ...
    if Position>=len(Event_list):
        ...
        if sv.Current_time>=time:
            ...
            event="P"+str(numcode)
            ch=[(event, st, time)]
            if code>Max_show_pins:                           # add offset event
...
def read_event_file(sv):                                                           
    ...
    global Data_file
    Event_list=[]
    fname=os.path.join(Datadir, Data_file+File_extension)
    try:
        with open(fname,"r") as file: 
            raw=file.read()                               # read all events
            line_list=raw.split("\n")                  # convert to  list
    except IOError:
        print("*** I/O error in file", fname,"***")
        return None
...
##            print(time, code)
    ...
    Event_list.append((time/Split_second+Exit_delay/Split_second, 99))
...
##        fname=os.path.join(Datadir, filename)         # directory is Data
        ...
        fname="Data\\essai.e01"             # full test     #  FOR TEST ONLY !!!
...
if __name__== "__main__":
    print("\n*** whand_io OFFLINE for Whand experiments ***")
    waitforuser()
