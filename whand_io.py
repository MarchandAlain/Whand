...
# The following functions are called by Whand, so must be present at least as dummies
#==============================================================
...
#===================================================== findlast
def findlast(text, target):                                                       # called by initbox
    """
...
def readpins(sv):                                                                   # required by Whand
    ...
    if (Printout or not Nosave) and Started[box]:                         # log or print results
        for nb, st, tim in ch:
            ...
            if nb[0]=="P":
                nb=-int(nb[1:])
                cd=Codes[nb] if nb in Codes else ("NA", "NA")
                msg+=cd[0] if st else cd[1]
            else:
                msg+=nb                                                 
...
##    global T0
    ...
    nb="K"+str(car)[2:-1]                                  # for compatibility with pins ("K")                               
...
##            xp=treejoin(v, spacing=Underscore)
